...
with open('requêtes/req4.sql') as query4: #on ouvre le ficher de la requête 4
    req4={"En quelle année est sortie le premier film Superman ?": query4.read()} #on met la requête 4 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql4=req4["En quelle année est sortie le premier film Superman ?"] #appel de la requête 4
execute_sql(conn,sql4) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
