...
with open('requêtes/req11.sql') as query11: #on ouvre le ficher de la requête 11
    req11={"Quels sont les titres des films notés plus de 9 sur 10 avec plus de 10 000 votes ?": query11.read()} #on met la requête 11 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql11=req11["Quels sont les titres des films notés plus de 9 sur 10 avec plus de 10 000 votes ?"] #appel de la requête 11
execute_sql(conn,sql11) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
