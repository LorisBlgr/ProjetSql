...
with open('requêtes/req6.sql') as query6: #on ouvre le ficher de la requête 6
    req6={"Quel est le métier d’Olivier Nakache ?": query6.read()} #on met la requête 6 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql6=req6["Quel est le métier d’Olivier Nakache ?"] #appel de la requête 6
execute_sql(conn,sql6) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
