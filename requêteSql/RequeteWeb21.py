...
with open('requêtes/req21.sql') as query21: #on ouvre le ficher de la requête 21
    req21={"Quels sont les noms des épisodes de Game of Thrones ?": query21.read()} #on met la requête 21 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql21=req21["Quels sont les noms des épisodes de Game of Thrones ?"] #appel de la requête 21
execute_sql(conn,sql21) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
