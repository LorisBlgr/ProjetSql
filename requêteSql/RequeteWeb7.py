...
with open('requêtes/req7.sql') as query7: #on ouvre le ficher de la requête 7
    req7={"Quels sont les films d'Olivier Nakache ?": query7.read()} #on met la requête 7 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql7=req7["Quels sont les films d'Olivier Nakache ?"] #appel de la requête 7
execute_sql(conn,sql7) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
