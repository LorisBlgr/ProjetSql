...
with open('requêtes/req17.sql') as query17: #on ouvre le ficher de la requête 17
    req17={"Quels sont les 5 films les plus longs ?": query17.read()} #on met la requête 17 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql17=req17["Quels sont les 5 films les plus longs ?"] #appel de la requête 17
execute_sql(conn,sql17) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
