...
with open('requêtes/req16.sql') as query16: #on ouvre le ficher de la requête 16
    req16={"Quel est le film le plus long ?": query16.read()} #on met la requête 16 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql16=req16["Quel est le film le plus long ?"] #appel de la requête 16
execute_sql(conn,sql16) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
