...
with open('requêtes/req5.sql') as query5: #on ouvre le ficher de la requête 5
    req5={"Quel est le titre original du film 'Les dents de la mer' ?": query5.read()} #on met la requête 5 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql5=req5["Quel est le titre original du film 'Les dents de la mer' ?"] #appel de la requête 5
execute_sql(conn,sql5) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
