...
with open('requêtes/req9.sql') as query9: #on ouvre le ficher de la requête 9
    req9={"Qui a écrit le scénario du film Taxi sorti en 1998 ?": query9.read()} #on met la requête 9 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql9=req9["Qui a écrit le scénario du film Taxi sorti en 1998 ?"] #appel de la requête 9
execute_sql(conn,sql9) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
