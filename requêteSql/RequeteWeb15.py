...
with open('requêtes/req15.sql') as query15: #on ouvre le ficher de la requête 15
    req15={"Quelle est la durée moyenne d’un film ?": query15.read()} #on met la requête 15 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql15=req15["Quelle est la durée moyenne d’un film ?"] #appel de la requête 15
execute_sql(conn,sql15) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
