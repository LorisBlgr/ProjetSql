...
with open('requêtes/req12.sql') as query12: #on ouvre le ficher de la requête 12
    req12={"Quelle sont les 5 comédies romantiques les mieux notées ?": query12.read()} #on met la requête 12 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql12=req12["Quelle sont les 5 comédies romantiques les mieux notées ?"] #appel de la requête 12
execute_sql(conn,sql12) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
