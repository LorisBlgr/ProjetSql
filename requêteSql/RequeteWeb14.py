...
with open('requêtes/req14.sql') as query14: #on ouvre le ficher de la requête 14
    req14={"Combien de films durent plus de 3 heures ?": query14.read()} #on met la requête 14 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql14=req14["Combien de films durent plus de 3 heures ?"] #appel de la requête 14
execute_sql(conn,sql14) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
