...
with open('requêtes/req2.sql') as query2: #on ouvre le ficher de la requête 2
    req2={"Combien y a-t-il de titres dans cette base de données ?": query2.read()} #on met la requête 2 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql1=req2["Combien y a-t-il de titres dans cette base de données ?"] #appel de la requête 2
