...
with open('requêtes/req1.sql') as query1: #on ouvre le ficher de la requête 1
    req1={"Quels sont les différents types de titres dans cette base de données ?": query1.read()} #on met la requête 1 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
def debuthtml():
    """
    Fonction permettant de structurer la page avec des balises html et des instructions css.
    """
    print("Content-type: text/html")
    print("\n")
    print("<html><head>")
    print("\n")
    print("<p id='quest'> ",''.join(req1),"</p>")
...
sql1=req1["Quels sont les différents types de titres dans cette base de données ?"] #appel de la requête 1
execute_sql(conn,sql1) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
