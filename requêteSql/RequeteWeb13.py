...
with open('requêtes/req13.sql') as query13: #on ouvre le ficher de la requête 13
    req13={"Quels sont les 10 films d’animation ayant reçu plus de 1000 votes les mieux notés ?": query13.read()} #on met la requête 12 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql13=req13["Quels sont les 10 films d’animation ayant reçu plus de 1000 votes les mieux notés ?"] #appel de la requête 13
execute_sql(conn,sql13) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
