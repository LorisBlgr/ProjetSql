...
with open('requêtes/req18.sql') as query18: #on ouvre le ficher de la requête 18
    req18={"Quels sont les titres des films les plus connus de Sean Connery ?": query18.read()} #on met la requête 18 dans un dictionnaire avec comme clé la question et comme valeur on lit la requête.
...
sql18=req18["Quels sont les titres des films les plus connus de Sean Connery ?"] #appel de la requête 18
execute_sql(conn,sql18) #execution de la fonction execute_sql prenant en argument la requête sql appelé précédement.
